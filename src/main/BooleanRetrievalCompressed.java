...
public class BooleanRetrievalCompressed extends Configured implements Tool {
    ...
    private void initialize(String indexPath, String collectionPath, FileSystem fs) throws IOException {
        ...
        FileStatus[] st = fs.listStatus(new Path(indexPath));
        partitionNo = st.length - 1;

        index = new MapFile.Reader[partitionNo];

        int arrayIndex = 0;
        for (int i = 0; i < st.length; i++) {
            if (st[i].getPath().toString().contains("SUCCESS"))
                continue;
            index[arrayIndex] = new MapFile.Reader(st[i].getPath(), fs.getConf());
            arrayIndex++;
        }
    ...
    private ArrayListWritable<PairOfInts> fetchPostings(String term) throws IOException {
        ...
        int freq = 0;
        for (int i = 0; i < df; i++) {
            lineNo += WritableUtils.readVInt(dis);
            freq = WritableUtils.readVInt(dis);
            pairs.add(new PairOfInts(lineNo, freq));
        }
