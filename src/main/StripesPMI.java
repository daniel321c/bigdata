...
public class StripesPMI extends Configured implements Tool {
    ...
    private static final class MyCombiner extends Reducer<Text, HMapStIW, Text, HMapStIW> {
        @Override
    ...
    private static final class MyReducer extends Reducer<Text, HMapStIW, Text, HashMap<String, PairOfFloatInt>> {
        private static HashMap<String, Integer> wordCounts = new HashMap<String, Integer>();
        private static int lineCount = 0;

        private static final HashMap<String, PairOfFloatInt> VALUE = new HashMap<String, PairOfFloatInt>();
        ...
        public void reduce(Text key, Iterable<HMapStIW> values, Context context)
            ...
            int countA = wordCounts.get(key.toString());

            VALUE.clear();

            for (Iterator<MapKI.Entry<String>> i = map.entrySet().iterator(); i.hasNext();) {
                MapKI.Entry<String> e = i.next();

                if (e.getValue() < threshold)
                    continue;

                int countB = wordCounts.get(e.getKey());
                double tmp = (e.getValue() * lineCount * 1.0d) / (countA * countB);
                double pmi = Math.log10(tmp);

                VALUE.put(e.getKey(), new PairOfFloatInt((float) pmi, e.getValue()));
            }
